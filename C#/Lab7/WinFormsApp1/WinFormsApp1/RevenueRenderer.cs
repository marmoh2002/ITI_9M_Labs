...
namespace ABCRevenueApp
{
    ...
    public class RevenueRenderer
    {
        ...
        {
            using (Font headFont = new Font("Verdana", 18, FontStyle.Bold))
            using (Font subFont = new Font("Verdana", 12, FontStyle.Italic | FontStyle.Underline))
            {
                float mid = w / 2;
                SizeF tSize = g.MeasureString("ABC Company", headFont);
                g.DrawString("ABC Company", headFont, Brushes.DarkBlue, mid - (tSize.Width / 2), 20);
